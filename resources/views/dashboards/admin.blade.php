            ...
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex gap-6 overflow-x-auto" aria-label="Tabs">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['tab' => 'accounts']) }}"
                        class="whitespace-nowrap border-b-2 py-3 px-1 text-sm font-medium transition {{ $tab === 'accounts' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        {{ __('User Accounts') }}
                    </a>
                    <a href="{{ request()->fullUrlWithQuery(['tab' => 'activity']) }}"
                        class="whitespace-nowrap border-b-2 py-3 px-1 text-sm font-medium transition {{ $tab === 'activity' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        {{ __('Activity Log') }}
                    </a>
                </nav>
            </div>
                    ...
                    <form method="GET" action="{{ route('dashboard.admin') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            @if (request()->anyFilled(['action', 'user_id', 'date_from', 'date_to']))
                                <a href="{{ request()->fullUrlWithQuery(['tab' => 'activity', 'action' => null, 'user_id' => null, 'date_from' => null, 'date_to' => null]) }}" class="text-sm text-gray-600 hover:text-gray-900 underline">{{ __('Clear') }}</a>
                            @endif
                        </div>
                    </form>
